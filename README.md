...
ArXiv 抓取遇到 HTTP 429 时，关闭底层库的短间隔重试，由数据源统一冷却重试（180、360、720、900、900 秒）；如果服务端返回更长的 `Retry-After`（秒数或 HTTP 日期），等待该时间后再请求。本次扫描中已完成的查询会复用，失败查询仍从头重试；耗尽重试后任务明确失败，不把缺失结果当成零篇论文。持续限流时应等待服务恢复，或将抓取任务迁移到符合 arXiv 使用规则的自有运行环境；这些机制不能保证上游服务始终可用。
